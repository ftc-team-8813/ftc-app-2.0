...
public class RightConeAuto extends LoggingOpMode{
    ...
    private final PID arm_lower = new PID(0.0351,0.0003,0.00091, 0.167,100,0.8);
    private final PID arm_upper = new PID(0.0598,0.00200,0.0028,0.11,100,0.8); // 0.029, 0.0022, 0.001 then 0.027, 0.00228
    ...
    public void loop() {
        ...
        cur_angles[2] += ftcDash.getClaw_increment();//-2.8125;
        ...
        lift.setLiftPower(Range.clip(al_pow,-0.34,0.34), Range.clip(au_pow,-0.34,0.34), wrist_pow);

        if (gamepad1.x || pressed_reset) {
            drivetrain.autoMove(0, 0, 0, 0, 1, 1, 0.07, odometry.getPose(), telemetry);
            pressed_reset = true;
        }
        else {
            switch (main_id) {
                case 0:
                    x = 400;
                    y = 45.5;
                    drivetrain.autoMove(59.26, -4.1, 0, 0, 1, 1, 3, odometry.getPose(), telemetry);
                    if (drivetrain.hasReached()) {
                        main_id += 1;
                    }
                    break;
                case 1:
                    drivetrain.autoMove(59.26, -4.1, 263.15, 0, 1, 1, 0.5, odometry.getPose(), telemetry);
                    if (drivetrain.hasReached()) {
                        lift_id += 1;
                        main_id += 1;
                        timer.reset();
                    ...
                    break;
                case 2:
                    drivetrain.autoMove(61.0, -4.1, 263.15, 0, 1, 1, 0.07, odometry.getPose(), telemetry);
                    break;
                case 3:
                    drivetrain.autoMove(24, -1, 0, 0, 1, 1, 0.5, odometry.getPose(), telemetry);
                    if (drivetrain.hasReached()) {
                        main_id += 1;
                    }
                    break;
            ...
            switch (lift_id) {
                case 0:
                    if (timer.seconds() > 0.6) {
                        x = -367;
                        y = 860;
                        if (al_error < 2.63 && au_error < 2.6 && wr_error < 5) {
                            lift_id += 1;
                            timer.reset();
                        }
                    }
                ...
                case 4:
                    if (timer.seconds() > 0.3) {
                        intake.setClaw(0.63);
                        if ((intake.getClawPosition() == 0.63) && (timer.seconds() > 1.4)) {
                            x = 757.9;
                            y = 250;
                            lift_id += 1;
                            timer.reset();
