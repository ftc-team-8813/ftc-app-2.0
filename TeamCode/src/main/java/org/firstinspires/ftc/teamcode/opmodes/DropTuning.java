...
public class DropTuning extends LoggingOpMode{
    ...
    private final Logger log = new Logger("Drop Auto");
...
//        ftcDash = new FTCDashboardValues();
    ...
    public void loop() {
        ...
        switch (main_id) {
            case 0:
                drivetrain.autoMove(0,0,0,0,0,0,0,odometry.getPose(),telemetry);
        }
        ...
        switch (lift_id) {
            ...
            case 2:
                if (timer.seconds() > 0.3) {
                    intake.setClaw(0.63);
                    if ((intake.getClawPosition() == 0.63) && (timer.seconds() > 0.9)) {
                        x = 757.9;
                        y = 250;
                        lift_id += 1;
                        timer.reset();
