...
@Autonomous(name = "Auto Tuning Auto")
public class AutoTuning extends LoggingOpMode{
    ...
    private final Logger log = new Logger("Tune Auto");
    ...
    public void loop() {
        ...
        switch (main_id) {
            case 0:
                x = 0;
                y = 860;
                drivetrain.autoMove(61, 5.5, 0, 0, 1, 1, 3, odometry.getPose(), telemetry);
                if (drivetrain.hasReached()) {
                    main_id += 1;
                }
                break;
            case 1:
                drivetrain.autoMove(61, 5.5, 96.85, 0, 1, 1, 0.5, odometry.getPose(), telemetry);
                if (drivetrain.hasReached()) {
                    lift_id += 1;
                    main_id += 1;
                }
                break;
            case 2:
                drivetrain.autoMove(61.0, 5.5, 96.85, 0, 1, 1, 0.07, odometry.getPose(), telemetry);
                break;
            case 3:
                drivetrain.autoMove(3.5,0,0,0,1,1,0.5, odometry.getPose(),telemetry);
                break;
        }
