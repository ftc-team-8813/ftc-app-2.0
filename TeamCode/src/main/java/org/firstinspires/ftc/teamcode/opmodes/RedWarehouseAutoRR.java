...
public class RedWarehouseAutoRR extends LoggingOpMode
{
    ...
    public void loop() {
        ...
        if (sequence == 1 && !drive.isBusy()) {
            ...
            if (robot.lineFinder.lineFound()) {
                drive.setPoseEstimate(new Pose2d(Status.TAPE_X_OFFSET, 0, drive.getExternalHeading()));
                sequence = 5;
            }
 ...
 //                   .turn(Math.toRadians(10))
            ...
            auto.height = 3;
            sequence = 14;
        } else if (sequence == 14 && auto.lift_dumped && auto.lift_dumped_timer.seconds() > Status.AUTO_DUMP_DRIVE_OFFSET && !drive.isBusy()) {
            ts2 = drive.trajectorySequenceBuilder(ts1.end())
                    .lineToLinearHeading(new Pose2d(58, 0, Math.toRadians(190)))
