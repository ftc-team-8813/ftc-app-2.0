...
public class ImageDraw
{
    ...
    public static final class Color
    {
        ...
        {
            if      (s.val.length == 4) return new Color((int)s.val[0], (int)s.val[1], (int)s.val[2], (int)s.val[3]);
            else if (s.val.length == 3) return new Color((int)s.val[0], (int)s.val[1], (int)s.val[2]);
            else if (s.val.length == 1) return new Color((int)s.val[0], (int)s.val[0], (int)s.val[0]);
    ...
    public static final class Lines extends Operation
    {
        ...
        {
            if (points == null || points.length < 2) throw new IllegalArgumentException("Must have at least 2 points for lines");
            if (fill == null)   this.fill   = NOCOLOR;
            else                this.fill   = fill;
